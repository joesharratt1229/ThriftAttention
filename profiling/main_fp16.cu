...
void fill_half_values(std::vector<half>& values, int seed)
{
    for (size_t i = 0; i < values.size(); ++i) {
        const float value =
            0.125f * static_cast<float>(((static_cast<int>(i) + seed) % 13) + 1);
                  ...
                  uint64_t** d_mma_ticks)
{
    ...
    fill_half_values(h_a, 1);
    fill_half_values(h_b, 7);
...
int main(int argc, char** argv)
{
    ...
    setup_inputs(max_blocks, &d_a, &d_b, &d_c, &d_mma_ticks);
    ...
    if (launch.auto_blocks) {
        for (int i = 0; i < WAVE_SWEEP_COUNT; ++i) {
            ...
            const BenchmarkResult result =
                run_benchmark(run_blocks, iters, d_a, d_b, d_c, d_mma_ticks,
                              start_event, stop_event);
            print_result(run_blocks, result);
        }
    } else {
        const BenchmarkResult result =
            run_benchmark(max_blocks, iters, d_a, d_b, d_c, d_mma_ticks,
                          start_event, stop_event);
