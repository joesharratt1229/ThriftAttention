...
template<typename T, bool CAUSAL>
static void fp4_attention_nvfp4_typed_hd256(
    const void* Q_raw,
    const void* K_raw,
    const void* V_raw,
    const void* S_Q_raw,
...
void fp4_attention_noncausal_nvfp4_hd256(
    ...
    bool is_bf16) {
    if (is_bf16) {
        fp4_attention_nvfp4_typed_hd256<__nv_bfloat16, false>(
            Q_raw, K_raw, V_raw, S_Q_raw, S_K_raw, S_V_raw, O_raw,
            bs, q_len, kv_len, kv_capacity, num_q_heads, num_kv_heads, head_dim);
    } else {
        fp4_attention_nvfp4_typed_hd256<half, false>(
            Q_raw, K_raw, V_raw, S_Q_raw, S_K_raw, S_V_raw, O_raw,
            bs, q_len, kv_len, kv_capacity, num_q_heads, num_kv_heads, head_dim);
    }
}
