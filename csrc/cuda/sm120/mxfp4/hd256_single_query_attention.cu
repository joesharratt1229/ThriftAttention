...
template<typename T>
static void thrift_attention_single_query_mxfp4_typed_hd256(
    ...
    int head_dim)
{
    ...
    if (total_kv_blocks < 128) {
        // CTA-cooperative. Larger contexts use split-KV to keep hd256 shared
        // memory below the per-block dynamic smem limit.
...
{
    if (is_bf16) {
        thrift_attention_single_query_mxfp4_typed_hd256<__nv_bfloat16>(
        ...
        thrift_attention_single_query_mxfp4_typed_hd256<half>(
            Q_fp16_raw, K_fp16_raw, V_fp16_raw, top_k, topk_count,
            Q_raw, K_raw, V_raw, S_Q_raw, S_K_raw, S_V_raw, O_raw,
            workspace_raw, bs, q_len, kv_len, kv_capacity, num_q_heads,
            num_kv_heads, head_dim);
