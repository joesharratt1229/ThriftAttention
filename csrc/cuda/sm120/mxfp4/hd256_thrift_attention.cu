...
__global__
void thrift_attention_kernel_hd256(
    const T* Q_fp16_in,
    const T* K_fp16_in,
    const T* V_fp16_in,
...
__launch_bounds__(NUM_WARPS * TA_WARP_SIZE)
__global__
void thrift_attention_fp16_finalize_kernel_hd256(
    const T* Q_fp16_in,
    const T* K_fp16_in,
    const T* V_fp16_in,
...
static void launch_thrift_attention_fp16_finalize_hd256(
    const T* Q_fp16,
    const T* K_fp16,
    const T* V_fp16,
    const int32_t* selected_blocks,
    ...
    int num_kv_heads)
{
    ...
    auto fp16_finalize_kernel = thrift_attention_fp16_finalize_kernel_hd256<
...
static void dispatch_thrift_attention_fp16_finalize_hd256(
    const T* Q_fp16,
    const T* K_fp16,
    const T* V_fp16,
    const int32_t* selected_blocks,
...
{
    if (topk_count <= 1)
        return launch_thrift_attention_fp16_finalize_hd256<T, CAUSAL, HEAD_DIM, BLOCK_Q, BLOCK_KV_FP4, 1>(
            Q_fp16, K_fp16, V_fp16, selected_blocks, topk_count, O,
            rowmax_state, rowsum_state, bs, q_len, kv_len, num_q_heads, num_kv_heads);
    if (topk_count <= 2)
        return launch_thrift_attention_fp16_finalize_hd256<T, CAUSAL, HEAD_DIM, BLOCK_Q, BLOCK_KV_FP4, 2>(
            Q_fp16, K_fp16, V_fp16, selected_blocks, topk_count, O,
            rowmax_state, rowsum_state, bs, q_len, kv_len, num_q_heads, num_kv_heads);
    if (topk_count <= 4)
        return launch_thrift_attention_fp16_finalize_hd256<T, CAUSAL, HEAD_DIM, BLOCK_Q, BLOCK_KV_FP4, 4>(
            Q_fp16, K_fp16, V_fp16, selected_blocks, topk_count, O,
            rowmax_state, rowsum_state, bs, q_len, kv_len, num_q_heads, num_kv_heads);
    if (topk_count <= 8)
        return launch_thrift_attention_fp16_finalize_hd256<T, CAUSAL, HEAD_DIM, BLOCK_Q, BLOCK_KV_FP4, 8>(
            Q_fp16, K_fp16, V_fp16, selected_blocks, topk_count, O,
            rowmax_state, rowsum_state, bs, q_len, kv_len, num_q_heads, num_kv_heads);
    if (topk_count <= 16)
        return launch_thrift_attention_fp16_finalize_hd256<T, CAUSAL, HEAD_DIM, BLOCK_Q, BLOCK_KV_FP4, 16>(
            Q_fp16, K_fp16, V_fp16, selected_blocks, topk_count, O,
            rowmax_state, rowsum_state, bs, q_len, kv_len, num_q_heads, num_kv_heads);
    if (topk_count <= 32)
        return launch_thrift_attention_fp16_finalize_hd256<T, CAUSAL, HEAD_DIM, BLOCK_Q, BLOCK_KV_FP4, 32>(
            Q_fp16, K_fp16, V_fp16, selected_blocks, topk_count, O,
            rowmax_state, rowsum_state, bs, q_len, kv_len, num_q_heads, num_kv_heads);
    if (topk_count <= 64)
        return launch_thrift_attention_fp16_finalize_hd256<T, CAUSAL, HEAD_DIM, BLOCK_Q, BLOCK_KV_FP4, 64>(
            Q_fp16, K_fp16, V_fp16, selected_blocks, topk_count, O,
            rowmax_state, rowsum_state, bs, q_len, kv_len, num_q_heads, num_kv_heads);
    if (topk_count <= 128)
        return launch_thrift_attention_fp16_finalize_hd256<T, CAUSAL, HEAD_DIM, BLOCK_Q, BLOCK_KV_FP4, 128>(
            Q_fp16, K_fp16, V_fp16, selected_blocks, topk_count, O,
            rowmax_state, rowsum_state, bs, q_len, kv_len, num_q_heads, num_kv_heads);
    if (topk_count <= 256)
        return launch_thrift_attention_fp16_finalize_hd256<T, CAUSAL, HEAD_DIM, BLOCK_Q, BLOCK_KV_FP4, 256>(
            Q_fp16, K_fp16, V_fp16, selected_blocks, topk_count, O,
            rowmax_state, rowsum_state, bs, q_len, kv_len, num_q_heads, num_kv_heads);
    if (topk_count <= 512)
        return launch_thrift_attention_fp16_finalize_hd256<T, CAUSAL, HEAD_DIM, BLOCK_Q, BLOCK_KV_FP4, 512>(
            Q_fp16, K_fp16, V_fp16, selected_blocks, topk_count, O,
            rowmax_state, rowsum_state, bs, q_len, kv_len, num_q_heads, num_kv_heads);
    return launch_thrift_attention_fp16_finalize_hd256<T, CAUSAL, HEAD_DIM, BLOCK_Q, BLOCK_KV_FP4, 2048>(
        Q_fp16, K_fp16, V_fp16, selected_blocks, topk_count, O,
        rowmax_state, rowsum_state, bs, q_len, kv_len, num_q_heads, num_kv_heads);
}

template<typename T, bool CAUSAL, int HEAD_DIM, int BLOCK_Q, int BLOCK_KV_FP4>
static void launch_thrift_attention_hd256(
    const T* Q_fp16,
    const T* K_fp16,
    const T* V_fp16,
    const int32_t* selected_blocks,
    ...
    int num_kv_heads)
{
    ...
    dispatch_thrift_attention_fp16_finalize_hd256<T, CAUSAL, HEAD_DIM, BLOCK_Q, BLOCK_KV_FP4>(
        Q_fp16, K_fp16, V_fp16,
        selected_blocks, topk_count,
        O, rowmax_state, rowsum_state,
        bs, q_len, kv_len, num_q_heads, num_kv_heads);
...
static void dispatch_thrift_attention_hd256(
    const T* Q_fp16,
    const T* K_fp16,
    const T* V_fp16,
    const int32_t* selected_blocks,
    ...
    int num_kv_heads)
{
    return launch_thrift_attention_hd256<T, CAUSAL, HEAD_DIM, 128, 128>(
...
static void thrift_attention_mxfp4_typed_hd256(
    ...
    int head_dim)
{
    ...
    dispatch_thrift_attention_hd256<T, CAUSAL, 256>(
        Q_fp16, K_fp16, V_fp16,
        selected_blocks, topk_count,
        topk_mask, topk_word_count,
        Q, K, V, S_Q, S_K, S_V, O,
...
{
    if (is_bf16) {
        thrift_attention_mxfp4_typed_hd256<__nv_bfloat16, true>(
        ...
        thrift_attention_mxfp4_typed_hd256<half, true>(
            Q_fp16_raw, K_fp16_raw, V_fp16_raw, selected_blocks_raw, topk_count,
            topk_mask_raw, topk_word_count, Q_raw, K_raw, V_raw, S_Q_raw, S_K_raw,
            S_V_raw, O_raw, rowmax_state_raw, rowsum_state_raw, bs, q_len,
            kv_len, kv_capacity, num_q_heads, num_kv_heads, head_dim);
...
{
    if (is_bf16) {
        thrift_attention_mxfp4_typed_hd256<__nv_bfloat16, false>(
            Q_fp16_raw, K_fp16_raw, V_fp16_raw, selected_blocks_raw, topk_count,
            topk_mask_raw, topk_word_count, Q_raw, K_raw, V_raw, S_Q_raw, S_K_raw,
            S_V_raw, O_raw, rowmax_state_raw, rowsum_state_raw, bs, q_len,
            kv_len, kv_capacity, num_q_heads, num_kv_heads, head_dim);
    } else {
        thrift_attention_mxfp4_typed_hd256<half, false>(
            Q_fp16_raw, K_fp16_raw, V_fp16_raw, selected_blocks_raw, topk_count,
            topk_mask_raw, topk_word_count, Q_raw, K_raw, V_raw, S_Q_raw, S_K_raw,
            S_V_raw, O_raw, rowmax_state_raw, rowsum_state_raw, bs, q_len,
            kv_len, kv_capacity, num_q_heads, num_kv_heads, head_dim);
