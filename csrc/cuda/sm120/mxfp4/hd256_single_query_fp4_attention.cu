...
template<typename T>
static void fp4_attention_single_query_mxfp4_typed_hd256(
    const void* Q_raw,
    const void* K_raw,
    const void* V_raw,
    const void* S_Q_raw,
    ...
    if (total_kv_blocks <= 4) {
        if (head_dim == 64)
            fp4_attention_single_query_nosplit_launch_hd256<T, 64>(Q, K, V, S_Q, S_K, S_V, O, bs, q_len, kv_len, kv_capacity, num_q_heads, num_kv_heads);
        else if (head_dim == 128)
            fp4_attention_single_query_nosplit_launch_hd256<T, 128>(Q, K, V, S_Q, S_K, S_V, O, bs, q_len, kv_len, kv_capacity, num_q_heads, num_kv_heads);
        else
        ...
        if (head_dim == 64)
            fp4_attention_single_query_cta_launch_hd256<T, 64>(Q, K, V, S_Q, S_K, S_V, O, bs, q_len, kv_len, kv_capacity, num_q_heads, num_kv_heads);
        else if (head_dim == 128)
            fp4_attention_single_query_cta_launch_hd256<T, 128>(Q, K, V, S_Q, S_K, S_V, O, bs, q_len, kv_len, kv_capacity, num_q_heads, num_kv_heads);
        else
...
void fp4_attention_single_query_mxfp4_hd256(
    ...
    bool is_bf16) {
    if (is_bf16) {
        fp4_attention_single_query_mxfp4_typed_hd256<__nv_bfloat16>(
        ...
        fp4_attention_single_query_mxfp4_typed_hd256<half>(
            Q_raw, K_raw, V_raw, S_Q_raw, S_K_raw, S_V_raw, O_raw, workspace_raw,
            bs, q_len, kv_len, kv_capacity, num_q_heads, num_kv_heads, head_dim);
    }
}
