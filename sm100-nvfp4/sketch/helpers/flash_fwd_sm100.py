...
class FlashAttentionForwardSm100:
    ...
    def __init__(
        ...
        # (128 x D) x (D x 128) block size
        self.m_block_size = m_block_size
        self.n_block_size = n_block_size
        # how tiles does one cta handle (in paper this defaults to 2)
        ...
        # whether to use async copy engine for epilogue warp. e.g load O from gmem to smem
        self.use_tma_O = (
        ...
        #softmax emulation is enabled for default head dim sizes i.e. wheee its expected to help.
        # enable_ex2_emu is derived: True if tuning config has freq > 0, else fallback to default logic
        
        """
        enable_ex2_emu:
            controls whether softmax may use software approx 2^x
            to reduce pressure on MUFU ex2.approx

        enabled for:
            hdim <= 128
            or hdim == 192 with 2CTA + noncausal + nonlocal

        disabled for:
            SM103
            hdim 192 causal/local
            hdim 192 without 2CTA
            larger/unvalidated sha
        """
        ...
        # whether to overlap loading of Q and o
        self.overlap_sO_sQ = (
        ...
        # Overwrites work allocation where stages is not default e,g 2
        if self.q_stage == 1:
        ...
        # layouting the tensor memory scratch columns
        # tmem cols 0..127     S0 = Q0 @ K^T scores, FP32
        # tmem cols 128..255   S1 = Q1 @ K^T scores, FP32
        # tmem cols 256..383   O0 = P0 @ V accumulator, FP32
        # tmem cols 384..511   O1 = P1 @ V accumulator, FP32
        ...
        # allocates number of registers for each warp/warpgroup type. 
        # set dynamically. softmax register/intensive heavy usage.
    ...
    def _setup_attributes(self):
        ...
        # .width returns bit width of the query tensor element type
        # so q_dtype.width // 8 converts bits per element into bytes per element
        # here we determine smem size of different sections
    ...
    def __call__(
        ...
        # (b, s_q, h, d)-> [s_q, d, h, b]
        # reinterprets to transposed layout preferred by cute
        ...
        # same cute reinterpretation for Output and LSE layout.
        ...
        ## overrides reg and warp organisation when fp8 - different 
        ## register pressures and layouts
        ...
        #cta_group = TWO if self.use_2cta_instrs else ONE
        #This chooses whether Blackwell 2CTA MMA instructions are used.
        #Then it builds two tiled MMA descriptions:
        #tiled_mma_qk  # Q @ K^T -> S
        #tiled_mma_pv  # P @ V   -> O
    ...
    def kernel(
        ...
        # whether use tma or cp.async
        # pipeline between tma and mma warp
        if const_expr(self.use_tma_Q):
        ...
        # pipeline between mma warp and softmax correction
        ...
        # once softmax is ready, pipeline between softmax and mma warp
        ...
        if const_expr(self.s0_s1_barrier and self.q_stage > 1):
            ...
            # Ensures no overlap on MUFU units betweeen softmax upper and softmax lower warpgroups
        ...
        #  LOAD
        # ///////////////////////////////////////////////////////////////////////////////
        # if warp index == ld warp indexes -> then load
    ...
    def softmax_loop(
        ...
        while work_tile.is_valid_tile:
            ...
            # increase in max that softmax will tolerate in log2e units 
            # before implementing rescale operation. 
            # e.g 2**8 grwoth
            rescale_threshold = (
    ...
    def softmax_step(
        ...
        # 1) returns ref max to subtract from the row fragment
        # 2) factor needed to rescale previously accumulated O and row_sum
    ...
    def epilogue_s2g(
        ...
        # epilogue loads output matrix from shared to gmem
