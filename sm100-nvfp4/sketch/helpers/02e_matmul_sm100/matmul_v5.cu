...
void matmul_v5_kernel(
  ...
  // tells us which part of the output tile each tb will compute on
  ...
  // NUM_STAGES is the pipeline depth of the main matmul loop -> K dimension is processed in chunks of size BLOCK_K
  // instead of load tile 0, compute tile 0 pattern load future tiles whilst computing current tiles
  // defines which stage i.e if stages =4, defines stages for tiles in flight

  // mbars is a chunk of shared memory used to store mbarrier objects
  // small synchronisation object 
  // allocates: num_stages barriers for tma, num_stages barriers for mma, 1 extra barrier for mainloop coordination
  ...
  if (warp_id == 0 && elect_sync()) {
    for (int i = 0; i < NUM_STAGES; i++) {
      //initiaises mbarrier object for tma, mma and main loop
  ...
  auto load = [&](int iter_k) {
    ...
    // waits on MMA done with this buffer
  ...
  for (int n = 0; n < BLOCK_N / 8; n++) {
    ...
    // Layout A
    float tmp[8];
    // tcgen05.ld asynchronously loads data from tensor memory at the location
    // specified by 32 bit address operand taddr into the destination register
    // r, collectively across all threads of the warp
    // all the threads in the warp must specify the same value of taddr 
    // which must be same base address across load operation
    // sync qualifier indicates that tcgen05.ld casues the executing thread
...
void matmul_v5_launch(
  ...
  auto this_kernel = matmul_v5_kernel<BLOCK_N, BLOCK_K, CTA_GROUP, NUM_STAGES, DO_PROFILE>;
  if (smem_size > 48'000)
...
void matmul_v5(
  ...
  matmul_v5_launch<256, 64, 2, 7, false>(A_ptr, B_ptr, C_ptr, M, N, K, nullptr, 0);
}

void profile_matmul_v5(
  const nv_bfloat16 *A_ptr,
  const nv_bfloat16 *B_ptr,
        nv_bfloat16 *C_ptr,
  int M, int N, int K,
  ...
  matmul_v5_launch<256, 64, 2, 7, true>(A_ptr, B_ptr, C_ptr, M, N, K, profiler_ptr, num_entries);
}
