...
// ---------------------------------------------------------------------------
// Optional instrumentation (FA4_PROF=1) and ablation (FA4_ABLATE_*=1) builds,
// driven by profile_breakdown.py.  All flags default OFF and compile to
// nothing: the production kernel is unchanged.
// ---------------------------------------------------------------------------
#ifndef FA4_PROF
#define FA4_PROF 0
#endif
// Softmax-warp tick granularity: the softmax warps ARE the critical path, so
// their ticks perturb the pipeline.  1 = wait-vs-busy only (2 ticks/iter,
// low perturbation, the default used by profile_breakdown.py); 2 = also
// split S-load / stats / convert (adds ~10-15% wall time -- use the ablation
// mode for an unperturbed view of that sub-structure).
#ifndef FA4_PROF_SM_LEVEL
#define FA4_PROF_SM_LEVEL 2
#endif
#ifndef FA4_ABLATE_SLOAD      // skip the S tmem score loads (scores read as 0)
#define FA4_ABLATE_SLOAD 0
#endif
#ifndef FA4_ABLATE_REDMAX     // skip the block-max reduction (constant maxima)
#define FA4_ABLATE_REDMAX 0
#endif
#ifndef FA4_ABLATE_CONVERT    // skip score->P conversion and the P/sf stores
#define FA4_ABLATE_CONVERT 0
#endif
#ifndef FA4_ABLATE_RESCALE    // never rescale O (lazy stats still maintained)
#define FA4_ABLATE_RESCALE 0
#endif
#ifndef FA4_ABLATE_KV_TMA     // fetch only K/V scale atoms, not the data tiles
#define FA4_ABLATE_KV_TMA 0
#endif
#ifndef FA4_ABLATE_MMA        // skip qk/pv MMA issue (commits still fire)
#define FA4_ABLATE_MMA 0
#endif
#ifndef FA4_ABLATE_TCCP       // skip the sf tcgen05.cp tmem copies
#define FA4_ABLATE_TCCP 0
#endif

// Per-(CTA, warp) cycle accumulators.  Slot = warp_id for softmax owners
// (0-3) and partners (4-7); 8 = mma, 9 = load, 10 = store.  Category meaning
// is per-role; the label tables live in profile_breakdown.py.  The last
// category is the warp's total span.
constexpr int PROF_MAX_CTAS = 160;
constexpr int PROF_SLOTS = 12;
constexpr int PROF_CATS = 12;
constexpr int PROF_SPAN = PROF_CATS - 1;

// Shared-memory layout, parameterised on the 2-CTA cluster variant: with
// CTA_PAIR the K/V data tiles are split along N across the pair, so each
// CTA's slot holds half the data (full sf atoms stay duplicated per CTA).
// Profiling accumulators live in smem (single-lane adds), not registers: the
// softmax paths sit at the spill boundary, and register accumulators spilled
// to local memory, inflating the very segments being measured.
template <int PAIR>
struct FA4Smem {
    static constexpr int KV_DATA = PAIR ? (KV_DATA_BYTES / 2) : KV_DATA_BYTES;
    static constexpr int KV_SLOT = KV_DATA + KV_SF_SLOT_BYTES;
    static constexpr int Q = 0;
    static constexpr int SF_Q = Q + Q_DATA_BYTES;
    static constexpr int KV = SF_Q + Q_SF_BYTES;
    static constexpr int O = KV + FA4_NUM_SLOTS * KV_SLOT;
    static constexpr int STATS = O + O_STAGE_BYTES;
    static constexpr int ONES = STATS + STATS_BYTES;
    static constexpr int ONES_SF = ONES + 512;
    static constexpr int PROF = ONES_SF + 512;
    static constexpr int BYTES = PROF + (FA4_PROF ? PROF_SLOTS * PROF_CATS * 4 : 0);
};
...
// OSUM gets 16 columns: the 2-CTA rowsum ride-along must run N=16 (the
// cta_group::2 MMA requires N >= 16, found the hard way), and only column 0
// is ever read.  The 1-CTA variant keeps N=8 and leaves cols 8-15 unused.
constexpr int TMEM_OSUM = 384;
constexpr int TMEM_SFQ = 400;
constexpr int TMEM_SFK0 = 408;
constexpr int TMEM_SFK1 = 416;
constexpr int TMEM_SFV0 = 424;
constexpr int TMEM_SFV1 = 432;
constexpr int TMEM_SFONES = 440;
constexpr int TMEM_COLS = 512;
constexpr int FA4_SUM_N = 8;

#if FA4_PROF
__device__ unsigned long long fa4_prof_buf[PROF_MAX_CTAS * PROF_SLOTS * PROF_CATS];

// Softmax owner/partner categories.  STATS bundles max-reduce + both pair
// barriers + rescale; EPI bundles the whole per-tile epilogue incl. its
// waits: 4 ticks per iteration keeps the perturbation of the critical
// softmax path low (finer splits measurably slowed the pipeline).
enum { PC_SM_WAIT_S = 0, PC_SM_SLOAD, PC_SM_STATS, PC_SM_CONVERT, PC_SM_EPI };
// MMA warp categories.
enum { PC_MMA_WAIT_QFULL = 0, PC_MMA_WAIT_K, PC_MMA_WAIT_SEMPTY,
       PC_MMA_QK_ISSUE, PC_MMA_WAIT_V, PC_MMA_WAIT_P0, PC_MMA_WAIT_P1,
       PC_MMA_WAIT_OEMPTY, PC_MMA_PV_ISSUE };
// Load warp categories.
enum { PC_LD_WAIT_QEMPTY = 0, PC_LD_WAIT_KEMPTY, PC_LD_ISSUE_K,
       PC_LD_WAIT_VEMPTY, PC_LD_ISSUE_V, PC_LD_ISSUE_Q };
// Store warp categories.
enum { PC_ST_WAIT_OEPI = 0, PC_ST_TMA };

// SM cycle counter, not %globaltimer: the S2R read is a few cycles instead
// of ~30 ns, which matters at ~4 reads per softmax iteration.  Fractions of
// a warp's span are exact; profile_breakdown.py converts to wall time.
__device__ __forceinline__ uint32_t prof_now()
{
    uint32_t t;
...
struct ProfAcc {
    ...
    __device__ __forceinline__ void start(uint32_t* prof_smem, int slot, bool writer_)
    {
        slot_ptr = prof_smem + slot * PROF_CATS;
        writer = writer_;
        t_start = t_prev = prof_now();
    }
    __device__ __forceinline__ void tick(int cat)
    {
        const uint32_t t = prof_now();
        if (writer) {
            slot_ptr[cat] += t - t_prev;
        }
        t_prev = t;
    }
    __device__ __forceinline__ void flush(int slot)
    {
        const uint32_t t = prof_now();
        if (writer && blockIdx.x < PROF_MAX_CTAS) {
            unsigned long long* p = fa4_prof_buf
                + (blockIdx.x * PROF_SLOTS + slot) * PROF_CATS;
            #pragma unroll
            for (int c = 0; c < PROF_SPAN; c++) {
                p[c] = slot_ptr[c];
            }
            p[PROF_SPAN] = t - t_start;
        }
    }
};
#define FA4_PROF_DECL                                                        \
    ProfAcc _prof;                                                           \
    _prof.start(reinterpret_cast<uint32_t*>(smem_storage + SM::PROF),        \
                warp_id < 8 ? warp_id                                        \
                            : (warp_id == 12 ? 8                             \
                                             : (warp_id == 14 ? 9           \
                                                              : (warp_id == 13 ? 10 : 11))), \
...
#define FA4_PROF_FLUSH(...)  _prof.flush(__VA_ARGS__)
#if FA4_PROF_SM_LEVEL >= 1
#define FA4_PROF_TICK_SM(cat)  _prof.tick(cat)
#else
#define FA4_PROF_TICK_SM(cat)  do {} while (0)
#endif
#if FA4_PROF_SM_LEVEL >= 2
#define FA4_PROF_TICK_SM2(cat) _prof.tick(cat)
#else
#define FA4_PROF_TICK_SM2(cat) do {} while (0)
#endif
#else
#define FA4_PROF_DECL        do {} while (0)
#define FA4_PROF_WRITER(b)   do {} while (0)
#define FA4_PROF_TICK(cat)   do {} while (0)
#define FA4_PROF_TICK_SM(cat)  do {} while (0)
#define FA4_PROF_TICK_SM2(cat) do {} while (0)
#define FA4_PROF_FLUSH(...)  do {} while (0)
#endif
...
enum class MbarId : int {
    ...
    DeallocPair = TmemDealloc + 1,   // 2-CTA: cross-CTA drain handshake
    Count = DeallocPair + 1
};
...
// ---- 2-CTA cluster primitives (validated in smoke_2cta.cu on this B200) ----
__device__ __forceinline__
uint32_t cluster_ctarank()
{
    uint32_t r;
...
// Map a local shared-memory address to CTA `rank`'s copy in the cluster
// address window.  Used for the pair leader's barriers (rank 0) and the
// dealloc handshake (rank^1).
__device__ __forceinline__
uint32_t mapa_shared(uint32_t addr, uint32_t rank)
{
    uint32_t r;
...
__device__ __forceinline__
void mbarrier_arrive_cluster(uint32_t remote_mbar)
{
    asm volatile(
...
__device__ __forceinline__
void mbarrier_arrive_cluster_count(uint32_t remote_mbar, uint32_t count)
{
    asm volatile(
        "mbarrier.arrive.release.cluster.shared::cluster.b64 _, [%0], %1;"
        :: "r"(remote_mbar), "r"(count)
        : "memory");
}
...
// 2-CTA commit: when the leader-issued cta_group::2 MMAs complete, arrive the
// mbarrier at this LOCAL offset in every CTA named by the mask (3 = the pair).
__device__ __forceinline__
void tcgen05_commit_mc(uint32_t mbar, uint16_t cta_mask)
{
    asm volatile(
...
{
    if constexpr (PAIR) {
        // Leader-issued; reads EACH CTA's own smem at the descriptor offset
        // and writes EACH CTA's own tmem (smoke-tested).
        asm volatile(
...
// Producer-side pair of tcgen05_fence_after: required between a warp's last
// tcgen05.st/ld and the mbarrier arrive that publishes it to another warp
// (wait::st/ld alone orders only same-thread tcgen05 ops).  Omitting it let
// the mma warp occasionally read stale P/sf words: nondeterministic output.
__device__ __forceinline__
...
// All tmem operands (D, tmem-A, SFA, SFB) of a cta_group::2 MMA are per-CTA-
// local: each CTA of the pair computes/holds its own 128 M-rows.  The smem B
// descriptor is likewise a local offset; each CTA holds its own N-half.
template <int PAIR = 0>
__device__ __forceinline__
...
// 2-CTA form: dst is the ISSUING CTA's local smem, but the completion (tx
// bytes) is signaled on the mbarrier named by `mbar_cluster` -- pass the
// mapa'd PAIR LEADER barrier.  The leader alone expects the pair total.
// (A plain local offset does NOT auto-route to the leader: smoke-tested.)
__device__ __forceinline__
void tma_2d_gmem2smem_pair(uint32_t dst, const void* tmap, int x, int y, uint32_t mbar_cluster, uint64_t cache_policy = EVICT_NORMAL)
{
    asm volatile(
        "cp.async.bulk.tensor.2d.shared::cluster.global.tile.mbarrier::complete_tx::bytes.L2::cache_hint.cta_group::2 "
        "[%0], [%1, {%2, %3}], [%4], %5;"
        :: "r"(dst), "l"(tmap), "r"(x), "r"(y), "r"(mbar_cluster), "l"(cache_policy)
        : "memory");
}
...
// The idesc M field is M>>4 at bits 24-28; (M>>7)<<27 produces the same bits
// for M in {128, 256}.  With CTA_PAIR the single leader-issued MMA covers
// M=256 (both CTAs' 128 rows).
template <int PAIR = 0>
__device__ __forceinline__
uint32_t fa4_qk_idesc()
{
    constexpr uint32_t M = PAIR ? 256 : FA4_MMA_M;
    return (1U << 7U)
        |  (1U << 10U)
...
{
    constexpr uint32_t M = PAIR ? 256 : FA4_MMA_M;
    // cta_group::2 requires N >= 16 (step 16); every output column is the
    // same row-sum, so widening is free and only column 0 is consumed.
    constexpr uint32_t N = PAIR ? 16 : FA4_SUM_N;
    return (1U << 7U)
        |  (1U << 10U)
        |  (N >> 3U << 17U)
        |  (M >> 7U << 27U);
...
// CTA_PAIR=0: the 1-CTA persistent kernel (unchanged behaviour).
// CTA_PAIR=1: 2-CTA cluster variant -- one M=256 QK/PV per pair issued by the
// leader CTA; K/V split along N across the pair (half data per CTA, sf atoms
// duplicated); leader-routed TMA completions; commit-multicast wakeups;
// remote P0/P1/OEmpty arrivals.  In pair mode the sf atoms travel via the
// sf tmaps (tensor TMA is the only bulk form whose completion can be routed
// to the pair leader -- smoke-tested); the raw atom pointers are unused.
template <int CTA_PAIR>
__global__
...
void nvfp4_sm100_attention_kernel(const __grid_constant__ CUtensorMap q_tmap,
                                  ...
                                  const __grid_constant__ CUtensorMap qsf_tmap,
                                  const __grid_constant__ CUtensorMap ksf_tmap,
                                  const __grid_constant__ CUtensorMap vsf_tmap,
                                  ...
                                  int num_tiles)
{
    using SM = FA4Smem<CTA_PAIR>;
    const int tid = threadIdx.x;
    const int warp_id = tid / WARP_SIZE;
    const int lane_id = tid & (WARP_SIZE - 1);
    const int q_per_kv = num_q_heads / num_kv_heads;
    const int kv_iters = kv_len / FA4_KV_TILE;
    const uint32_t pair_rank = CTA_PAIR ? cluster_ctarank() : 0;
    const bool pair_leader = pair_rank == 0;
    ...
    float* hmax1 = reinterpret_cast<float*>(smem_storage + SM::STATS);
    ...
    if (warp_id == 0 && elect_sync()) {
        ...
        // With CTA_PAIR, P0/P1Ready and OEmpty live on the LEADER and are
        // remote-arrived by BOTH CTAs' softmax lanes: counts double.
        constexpr int ARR = CTA_PAIR ? 2 : 1;
        for (int i = 0; i < FA4_S_BUFS; i++) {
            ...
            mbarrier_init(mbar_base + mbar_offset(MbarId::P0Ready, i), ARR * FA4_Q_STAGE_ROWS);
            // Partners' P chunk 1 plus the owners' rescale-done arrivals.
            mbarrier_init(mbar_base + mbar_offset(MbarId::P1Ready, i), ARR * 2 * FA4_Q_STAGE_ROWS);
        }
        mbarrier_init(mbar_base + mbar_offset(MbarId::OFull), 1);
        mbarrier_init(mbar_base + mbar_offset(MbarId::OEpi), FA4_Q_STAGE_ROWS);
        mbarrier_init(mbar_base + mbar_offset(MbarId::OEmpty), ARR * FA4_Q_STAGE_ROWS);
        ...
        mbarrier_init(mbar_base + mbar_offset(MbarId::DeallocPair), 1);
        asm volatile("fence.mbarrier_init.release.cluster;" ::: "memory");
    }

    if (warp_id == 12) {
        tcgen05_alloc<CTA_PAIR>(tmem_addr_smem, TMEM_COLS);
        tcgen05_relinquish_alloc_permit<CTA_PAIR>();
    }

    if (warp_id == 13) {
        uint32_t* ones_data = reinterpret_cast<uint32_t*>(smem_storage + SM::ONES);
        uint32_t* ones_sf = reinterpret_cast<uint32_t*>(smem_storage + SM::ONES_SF);
        for (int i = lane_id; i < 128; i += WARP_SIZE) {
            ones_data[i] = 0x22222222u;  // fp4 e2m1 1.0 pairs
            ones_sf[i] = 0x38383838u;    // e4m3 1.0
        }
#if FA4_PROF
        uint32_t* prof_zero = reinterpret_cast<uint32_t*>(smem_storage + SM::PROF);
        for (int i = lane_id; i < PROF_SLOTS * PROF_CATS; i += WARP_SIZE) {
            prof_zero[i] = 0;
        }
#endif
    ...
    if constexpr (CTA_PAIR) {
        // All threads still present: cross-CTA visibility of the freshly
        // initialised mbarriers before any remote arrive / leader-routed TMA.
        cluster_sync();
    }
    ...
    // Persistent tile schedule.  CTA_PAIR: each cluster strides over 256-row
    // SUPER-tiles (two adjacent 128-row q-blocks of the same head); CTA rank
    // r keeps its own 128 rows, so all per-CTA addressing is unchanged.
    const int q_blocks_256 = q_blocks_128 >> 1;
    const int tile_first = CTA_PAIR ? static_cast<int>(blockIdx.x >> 1)
                                    : static_cast<int>(blockIdx.x);
    const int tile_stride = CTA_PAIR ? static_cast<int>(gridDim.x >> 1)
                                     : static_cast<int>(gridDim.x);
    const int tile_limit = CTA_PAIR ? (num_tiles >> 1) : num_tiles;
    auto gtile_of = [&](int t) -> int {
        if constexpr (CTA_PAIR) {
            const int qb2 = t % q_blocks_256;
            const int bh = t / q_blocks_256;
            return bh * q_blocks_128 + 2 * qb2 + static_cast<int>(pair_rank);
        }
        return t;
    };
    set_tile(gtile_of(tile_first));

    // Profiling accumulators (FA4_PROF builds): shared by reference with the
    // role lambdas below; each role ticks its own categories.
    FA4_PROF_DECL;

    auto q_smem = [&]() -> uint32_t { return smem + SM::Q; };
    auto sf_q_smem = [&]() -> uint32_t { return smem + SM::SF_Q; };
    auto kv_slot_smem = [&](int slot) -> uint32_t {
        return smem + SM::KV + slot * SM::KV_SLOT;
    };
    auto kv_slot_sf_smem = [&](int slot) -> uint32_t {
        return kv_slot_smem(slot) + SM::KV_DATA;
    };
    auto o_stage_smem = [&]() -> uint32_t { return smem + SM::O; };
    auto o_stage_ptr = [&]() -> __nv_bfloat16* {
        return reinterpret_cast<__nv_bfloat16*>(smem_storage + SM::O);
    };
    ...
    // sf-atom coordinates in the qsf/ksf/vsf tmaps (pair mode): the tmaps use
    // 256-byte rows (TMA box cap), so a tile's two atoms are 4 rows.
    auto q_sf_row = [&]() -> int { return (q_sf_head_base + q_block) * FA4_QK_K_ITERS * 2; };
    auto k_sf_row = [&](int kv_iter) -> int { return (k_sf_head_base + kv_iter) * FA4_QK_K_ITERS * 2; };
    auto v_sf_row = [&](int kv_iter) -> int { return (v_sf_head_base + kv_iter * FA4_PV_K_ITERS) * 2; };

    auto issue_q = [&]() {
        const uint32_t mbar = mbar_base + mbar_offset(MbarId::QFull);
        if constexpr (CTA_PAIR) {
            // Each CTA loads its own 128 Q rows + sf; completions land on the
            // pair leader's QFull, which alone expects the pair total.
            const uint32_t leader_mbar = mapa_shared(mbar, 0);
            if (pair_leader) {
                mbarrier_arrive_expect_tx(mbar, 2 * (Q_DATA_BYTES + Q_SF_BYTES));
            }
            tma_2d_gmem2smem_pair(q_smem(), &q_tmap, 0, q_global_row_base, leader_mbar);
            tma_2d_gmem2smem_pair(sf_q_smem(), &qsf_tmap, 0, q_sf_row(), leader_mbar);
        } else {
    ...
    auto issue_k = [&](int slot, int g, int kv_iter) {
        ...
        if constexpr (CTA_PAIR) {
            // Split along N: CTA rank r loads kv rows [64r, 64r+64) of the
            // tile (halved tmap box); full sf atoms duplicated per CTA.
            const uint32_t leader_mbar = mapa_shared(mbar, 0);
            const int kv_row = kv_global_row_base + kv_iter * FA4_KV_TILE
                + static_cast<int>(pair_rank) * (FA4_KV_TILE / 2);
            if (pair_leader) {
                mbarrier_arrive_expect_tx(
                    mbar, (FA4_ABLATE_KV_TMA ? 0 : KV_DATA_BYTES) + 2 * K_SF_BYTES);
            }
#if !FA4_ABLATE_KV_TMA
            tma_2d_gmem2smem_pair(kv_slot_smem(slot), &k_tmap, 0, kv_row, leader_mbar);
#else
            (void)kv_row;
#endif
            tma_2d_gmem2smem_pair(kv_slot_sf_smem(slot), &ksf_tmap, 0, k_sf_row(kv_iter), leader_mbar);
        } else {
            const int kv_row = kv_global_row_base + kv_iter * FA4_KV_TILE;
#if FA4_ABLATE_KV_TMA
            (void)kv_row;
            tma_gmem2smem(kv_slot_sf_smem(slot), k_sf_src(kv_iter), K_SF_BYTES, mbar);
            mbarrier_arrive_expect_tx(mbar, K_SF_BYTES);
#else
    ...
    auto issue_v = [&](int slot, int g, int kv_iter) {
        ...
        const uint32_t mbar = mbar_base + mbar_offset(MbarId::KVFull, slot);
        if constexpr (CTA_PAIR) {
            // Split along N = head-dim: CTA rank r loads V^T rows [64r, 64r+64).
            const uint32_t leader_mbar = mapa_shared(mbar, 0);
            const int v_row = v_global_row_base
                + static_cast<int>(pair_rank) * (FA4_HEAD_DIM / 2);
            if (pair_leader) {
                mbarrier_arrive_expect_tx(
                    mbar, (FA4_ABLATE_KV_TMA ? 0 : KV_DATA_BYTES) + 2 * V_SF_BYTES);
            }
#if !FA4_ABLATE_KV_TMA
            tma_2d_gmem2smem_pair(kv_slot_smem(slot), &v_tmap,
                                  kv_iter * FA4_KV_TILE, v_row, leader_mbar);
#else
            (void)v_row;
#endif
            tma_2d_gmem2smem_pair(kv_slot_sf_smem(slot), &vsf_tmap, 0, v_sf_row(kv_iter), leader_mbar);
        } else {
#if FA4_ABLATE_KV_TMA
            tma_gmem2smem(kv_slot_sf_smem(slot), v_sf_src(kv_iter), V_SF_BYTES, mbar);
            mbarrier_arrive_expect_tx(mbar, V_SF_BYTES);
#else
    ...
    auto copy_sf_q_to_tmem = [&]() {
        ...
        for (int d = 0; d < FA4_QK_K_ITERS; d++) {
            tcgen05_cp<CTA_PAIR>(tmem_base + TMEM_SFQ + d * 4, sf_desc + (uint64_t)d * (SF_ATOM_BYTES >> 4));
        }
    };
    ...
    auto copy_sf_k_to_tmem = [&](int slot, int buffer) {
        ...
        #pragma unroll
        for (int d = 0; d < FA4_QK_K_ITERS; d++) {
            tcgen05_cp<CTA_PAIR>(sf_k_tmem(buffer) + d * 4, sf_desc + (uint64_t)d * (SF_ATOM_BYTES >> 4));
        }
    };
    ...
    auto copy_sf_v_to_tmem = [&](int slot, int buffer) {
        ...
        for (int c = 0; c < FA4_PV_K_ITERS; c++) {
            tcgen05_cp<CTA_PAIR>(sf_v_tmem(buffer) + c * 4, sf_desc + (uint64_t)c * (SF_ATOM_BYTES >> 4));
        }
    };
    ...
    auto qk_gemm = [&](int buf, int k_slot, int sf_k_buffer) {
        ...
        for (int d = 0; d < FA4_QK_K_ITERS; d++) {
            ...
            tcgen05_mma_nvfp4<CTA_PAIR>(
                s_tmem(buf),
                q_desc,
                k_desc,
                fa4_qk_idesc<CTA_PAIR>(),
    ...
    auto pv_gemm = [&](int buf, int chunk, int v_slot, int sf_v_buffer, bool accumulate) {
        ...
        tcgen05_mma_nvfp4_pv<CTA_PAIR>(
            o_tmem(),
            p_tmem(buf, chunk),
            v_desc,
            fa4_pv_idesc<CTA_PAIR>(),
            sf_p_tmem(buf, chunk),
        ...
        tcgen05_mma_nvfp4_pv<CTA_PAIR>(
            o_sum_tmem(),
            p_tmem(buf, chunk),
            make_desc_data_32b(smem + SM::ONES),
            fa4_sum_idesc<CTA_PAIR>(),
            sf_p_tmem(buf, chunk),
    ...
    // P0/P1Ready and OEmpty are consumed by the pair leader's mma warp: with
    // CTA_PAIR the whole warp's arrivals cross the cluster fabric as ONE
    // count-32 arrive from an elected lane (per-lane remote arrives are far
    // too slow).  Callers fence tmem work BEFORE calling; the __syncwarp
    // inside orders all lanes' prior work with the elected lane's arrive.
    auto arrive_pair_leader = [&](MbarId id, int index) {
        const uint32_t addr = mbar_base + mbar_offset(id, index);
        if constexpr (CTA_PAIR) {
            __syncwarp();
            if (elect_sync()) {
                mbarrier_arrive_cluster_count(mapa_shared(addr, 0), WARP_SIZE);
            }
        } else {
            mbarrier_arrive(addr);
        }
    };
    ...
    auto softmax_owner = [&]() {
        ...
        for (int lt = 0, t = tile_first; t < tile_limit;
             lt++, t += tile_stride) {
            ...
            for (int iter = 0; iter < kv_iters; iter++) {
                ...
                FA4_PROF_TICK_SM(PC_SM_WAIT_S);
                tcgen05_fence_after();
#if !FA4_ABLATE_SLOAD
                tcgen05_ld_32x32bx64(scores, s_tmem(buf) + row_tmem);
#endif
                FA4_PROF_TICK_SM2(PC_SM_SLOAD);
#if FA4_ABLATE_REDMAX
                #pragma unroll
                for (int b = 0; b < 4; b++) {
                    block_row_max[b] = TA_MAGIC;
                }
                const float half_max = TA_MAGIC;
#else
                ...
                // Rescale must be ordered after PV(g-1)'s writes to O.  SFull(g)
                // does NOT imply that: the QK-prefetch hoist issues QK(g) before
                // PV(g-1), so SFull(g)'s commit tracks only ops up to QK(g).
                // SEmpty(buf(g-1)) is committed right after PV1(g-1): wait for it
                ...
                if (iter > 0 && __any_sync(0xffffffffu, acc_scale != 1.0f)) {
                    // Parity = phase BEFORE commit(pg) = (pg>>1)&1, so the
                    // wait blocks exactly until that commit flips the phase.
                    const int pg = g - 1;
                    mbarrier_wait(mbar_base + mbar_offset(MbarId::SEmpty, pg & 1),
                                  (pg >> 1) & 1);
                    tcgen05_fence_after();
                    rescale_o(acc_scale);
                }
                ...
                arrive_pair_leader(MbarId::P0Ready, buf);
                arrive_pair_leader(MbarId::P1Ready, buf);
                FA4_PROF_TICK_SM(PC_SM_CONVERT);
            }
            gi += kv_iters;
            ...
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            // OEmpty publishes the O/O_sum tmem reads: the mma warp must not
            // overwrite O until every owner's tcgen05.ld has actually landed.
            tcgen05_fence_before();
            mbarrier_arrive(mbar_base + mbar_offset(MbarId::OEpi));
            arrive_pair_leader(MbarId::OEmpty, 0);
            FA4_PROF_TICK_SM(PC_SM_EPI);
        }

        FA4_PROF_FLUSH(warp_id);
    ...
    auto softmax_partner = [&]() {
        ...
        for (int t = tile_first; t < tile_limit; t += tile_stride) {
            for (int iter = 0; iter < kv_iters; iter++) {
                const int g = gi + iter;
                const int buf = g & 1;
                const int ph = (g >> 1) & 1;
    ...
    auto mma_loop = [&]() {
        ...
        // In pair mode only the leader CTA issues MMAs; its commits multicast
        // to the same barrier offset in BOTH CTAs of the pair.
        const bool mma_leader = !CTA_PAIR || pair_leader;
        auto commit_pair = [&](MbarId id, int index = 0) {
            const uint32_t addr = mbar_base + mbar_offset(id, index);
            if constexpr (CTA_PAIR) {
                tcgen05_commit_mc(addr, 3);
            } else {
                tcgen05_commit(addr);
            }
        };

        if (elected && mma_leader) {
            FA4_PROF_WRITER(true);
            tcgen05_cp<CTA_PAIR>(tmem_base + TMEM_SFONES, make_desc_sf(smem + SM::ONES_SF));

            int gi = 0;
            for (int lt = 0, t = tile_first; t < tile_limit;
                 lt++, t += tile_stride) {
                mbarrier_wait(mbar_base + mbar_offset(MbarId::QFull), lt & 1);
                FA4_PROF_TICK(PC_MMA_WAIT_QFULL);
                tcgen05_fence_after();
                copy_sf_q_to_tmem();
                FA4_PROF_TICK(PC_MMA_QK_ISSUE);
                ...
                // First QK of the tile.
                {
                    ...
                    FA4_PROF_TICK(PC_MMA_WAIT_SEMPTY);
                    tcgen05_fence_after();
                    qk_gemm(buf, k_slot, buf);
                    commit_pair(MbarId::SFull, buf);
                    commit_pair(MbarId::KVEmpty, k_slot);
                    FA4_PROF_TICK(PC_MMA_QK_ISSUE);
                }
                ...
                for (int iter = 0; iter < kv_iters; iter++) {
                    ...
                    FA4_PROF_TICK(PC_MMA_WAIT_P1);
                    tcgen05_fence_after();
                    pv_gemm(buf, 1, v_slot, vbuf, true);
                    commit_pair(MbarId::SEmpty, buf);
                    commit_pair(MbarId::KVEmpty, v_slot);
                    FA4_PROF_TICK(PC_MMA_PV_ISSUE);
                }
                gi += kv_iters;

                commit_pair(MbarId::QEmpty);
                commit_pair(MbarId::OFull);
                FA4_PROF_TICK(PC_MMA_PV_ISSUE);
            }
        ...
        if (elected) {
            // Own CTA's owners have finished every tmem read (epilogue done).
            mbarrier_wait(mbar_base + mbar_offset(MbarId::TmemDealloc), 0);
        }
        __syncwarp();
        if constexpr (CTA_PAIR) {
            // Cross-CTA drain handshake before dealloc: the leader-issued
            // MMAs write BOTH CTAs' tmem, so neither side may free until the
            // peer's owners are also done.  (tmem_allocator pattern: arrive
            // the PEER's barrier, wait your OWN.)
            if (elect_sync()) {
                mbarrier_arrive_cluster(mapa_shared(
                    mbar_base + mbar_offset(MbarId::DeallocPair), pair_rank ^ 1u));
                mbarrier_wait(mbar_base + mbar_offset(MbarId::DeallocPair), 0);
            }
            __syncwarp();
        }
        tcgen05_dealloc<CTA_PAIR>(tmem_base, TMEM_COLS);
    };
                                       ...
                                       int num_kv_heads)
{
    ...
    static_assert(FA4Smem<0>::BYTES <= 227 * 1024, "1-CTA smem layout too large");
    static_assert(FA4Smem<1>::BYTES <= 227 * 1024, "2-CTA smem layout too large");
...
// sf-atom tensor map: rows of 512-byte atoms, fetched two rows per tile.
// Only the 2-CTA variant uses these (tensor TMA is the one bulk form whose
// completion can be routed to the pair leader's mbarrier).
__host__
cudaError_t fa4_encode_sf_tmap(CUtensorMap* tmap,
                               const uint8_t* ptr,
                               uint64_t total_atoms)
{
    // TMA box dims cap at 256 elements: express the atoms as 256-byte rows
    // (2 rows per atom), 4 rows per fetched tile (= 2 atoms = 1024 B).
    constexpr uint32_t rank = 2;
    uint64_t global_dim[rank] = {SF_ATOM_BYTES / 2, total_atoms * 2};
    uint64_t global_strides[rank - 1] = {SF_ATOM_BYTES / 2};
    uint32_t box_dim[rank] = {SF_ATOM_BYTES / 2, 4};
    uint32_t element_strides[rank] = {1, 1};
    ...
    return fa4_cu_result(err, "cuTensorMapEncodeTiled(sf)");
}

// 2-CTA dispatch: q_len must pair 128-row blocks, and small grids stay on
// the 1-CTA variant (256-row effective tiles double the wave-quantisation
// tail).  FA4_FORCE_PAIR=0/1 overrides for A/B testing; FA4_PAIR_MIN_TILES
// tunes the crossover (default 8 waves of cluster slots).
__host__
bool fa4_use_pair(int num_tiles, int q_len, int sm_count)
{
    if ((q_len % (2 * FA4_Q_TILE_ROWS)) != 0 || (num_tiles & 1) != 0) {
        return false;
    }
    if (const char* f = getenv("FA4_FORCE_PAIR")) {
        if (f[0] != '\0') {
            return atoi(f) != 0;
        }
    }
    // Opt-in while the pair variant trails the 1-CTA kernel: dispatch to it
    // only when FA4_PAIR_MIN_TILES (or FA4_FORCE_PAIR=1) says so.
    long min_tiles = 1L << 40;
    if (const char* e = getenv("FA4_PAIR_MIN_TILES")) {
        if (e[0] != '\0') {
            min_tiles = atol(e);
        }
    }
    return num_tiles >= min_tiles;
}
                                         ...
                                         cudaStream_t stream)
{
    ...
    const bool pair = fa4_use_pair(num_tiles, q_len, sm_count);
    ...
    // 2-CTA: K/V boxes halve (each CTA of the pair fetches its own N-half).
    err = fa4_encode_fp4_tmap(&k_tmap, k, FA4_HEAD_DIM, total_kv_rows,
                              FA4_HEAD_DIM, pair ? FA4_KV_TILE / 2 : FA4_KV_TILE,
                              CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_64B);
    if (err != cudaSuccess) {
        return err;
    }
    err = fa4_encode_fp4_tmap(&v_tmap, v_t, kv_len, total_v_rows,
                              FA4_KV_TILE, pair ? FA4_HEAD_DIM / 2 : FA4_HEAD_DIM,
                              CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_64B);
    if (err != cudaSuccess) {
        return err;
    }
    ...
    CUtensorMap qsf_tmap;
    CUtensorMap ksf_tmap;
    CUtensorMap vsf_tmap;
    const uint64_t bh_q = static_cast<uint64_t>(batch) * num_q_heads;
    const uint64_t bh_kv = static_cast<uint64_t>(batch) * num_kv_heads;
    err = fa4_encode_sf_tmap(&qsf_tmap, sf_q_atoms,
                             bh_q * (q_len / FA4_Q_STAGE_ROWS) * FA4_QK_K_ITERS);
    if (err != cudaSuccess) {
        return err;
    }
    err = fa4_encode_sf_tmap(&ksf_tmap, sf_k_atoms,
                             bh_kv * (kv_len / FA4_KV_TILE) * FA4_QK_K_ITERS);
    if (err != cudaSuccess) {
        return err;
    }
    err = fa4_encode_sf_tmap(&vsf_tmap, sf_v_atoms,
                             bh_kv * (kv_len / 64));
    if (err != cudaSuccess) {
        return err;
    }

    auto kernel = pair ? nvfp4_sm100_attention_kernel<1>
                       : nvfp4_sm100_attention_kernel<0>;
    const int smem_bytes = pair ? FA4Smem<1>::BYTES : FA4Smem<0>::BYTES;
    ...
    if (pair) {
        const int pairs = num_tiles / 2;
        const int clusters = pairs < sm_count / 2 ? pairs : sm_count / 2;
        cudaLaunchConfig_t cfg = {};
        cfg.gridDim = dim3(2 * clusters);
        cfg.blockDim = dim3(FA4_THREADS);
        ...
        cfg.attrs = attrs;
        cfg.numAttrs = 1;
        err = cudaLaunchKernelEx(&cfg, kernel,
                                 q_tmap, k_tmap, v_tmap, o_tmap,
                                 qsf_tmap, ksf_tmap, vsf_tmap,
                                 sf_q_atoms, sf_k_atoms, sf_v_atoms,
                                 q_len, kv_len, num_q_heads, num_kv_heads,
                                 softmax_scale_log2, v_descale, num_tiles);
        if (err != cudaSuccess) {
            return err;
        }
    } else {
...
// Profiling-buffer accessors: always exported so the extension links in every
// build; no-ops (enabled=0) unless compiled with -DFA4_PROF=1.
extern "C"
void nvfp4_sm100_prof_info(int* enabled, int* ctas, int* slots, int* cats)
{
    *enabled = FA4_PROF;
    *ctas = PROF_MAX_CTAS;
    *slots = PROF_SLOTS;
...
extern "C"
cudaError_t nvfp4_sm100_prof_reset(cudaStream_t stream)
{
#if FA4_PROF
    void* ptr = nullptr;
    cudaError_t err = cudaGetSymbolAddress(&ptr, fa4_prof_buf);
    if (err != cudaSuccess) {
        return err;
    }
    return cudaMemsetAsync(ptr, 0, sizeof(fa4_prof_buf), stream);
#else
    (void)stream;
    return cudaSuccess;
#endif
...
extern "C"
cudaError_t nvfp4_sm100_prof_fetch(unsigned long long* dst, cudaStream_t stream)
{
#if FA4_PROF
    void* ptr = nullptr;
    cudaError_t err = cudaGetSymbolAddress(&ptr, fa4_prof_buf);
    ...
    return cudaMemcpyAsync(dst, ptr, sizeof(fa4_prof_buf),
